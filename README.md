...
### 프로젝트 배경 : 프로젝트로 성장하기🌱
개인 프로젝트를 통해, 기획에서 배포까지의 모든 과정을 직접 경험하며 문제를 해결하고 성장하고자 했습니다. <br>
프로젝트는 단계별로 발전하며 지속적으로 개선되었습니다. <br>
이번 SanrioShop 프로젝트는 V3 버전입니다.  <br>
V2에서 구현한 검색 방식의 한계를 느끼고, 사용자 경험을 개선하기 위해 리팩토링을 진행했습니다. <br>
실제 애플리케이션 "콜리"를 레퍼런스로 삼아, 태그 및 테마 기반의 검색 기능을 도입했으며, 배포 과정에서 자동화된 배포 프로세스를 구축했습니다. <br>
+ V1: 처음에는 로컬 환경에서 기본적인 쇼핑몰 애플리케이션을 개발
+ V2: V1에서 부족했던 점을 개선하기 위해 리팩토링을 진행하고 배포까지

> [쇼핑몰 프로젝트 V2 - Github](https://github.com/Kim-Gyuri/bookstore) <br>
> [쇼핑몰 프로젝트 V1 - Github](https://github.com/Kim-Gyuri/SpringBoot-Online-Shopping-Store) 
...
+ 비관적 락과 트랜잭션으로 주문 처리 시 동시성 제어 구현
+ 태그 및 테마 기반 검색 기능 구현

## 성과
+  단일 토큰에서 다중 토큰 구조(Refresh 토큰 포함)로 보안을 강화하고, 토큰의 생명 주기를 관리하는 방식으로 보안 처리 개선
+  태그 및 테마 기반 검색 기능 구현
+  동시 주문 요청 처리 시 트랜잭션과 예외 처리
+  프로젝트 Wiki 작성(레퍼런스 분석부터 성능, 구조, 구현, 테스트에 대한 전반적인 문서화)
+  GitHub Actions와 Docker를 활용한 CI/CD 파이프라인 구축 및 컨테이너 기반 배포 자동화
...
LEFT JOIN으로 쿼리를 작성했지만, Postman으로 API를 테스트한 결과, <br>
1:N 관계에서 발생한 중복 데이터로 인해 예상했던 12개의 항목 대신 6개의 고유 항목만 반환되었다.  <br>
이는 중복된 데이터가 페이징 과정에서 필터링되지 않아 일부 데이터가 누락된 결과로 보입니다.<br><br>

> (이제 item과 itemImg가 1:N 관계로 LEFT JOIN 할 때, <br>
> 하나의 item에 여러 개의 itemImg가 연결되면, 쿼리 결과에서 동일한 item 데이터가 여러 번 반환됨을 이해했다.<br>
